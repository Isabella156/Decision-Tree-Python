...
def deviation(value,parent_pos,parent_neg):
    ...
    predict = value.pos + value.neg

    expectPos = parent_pos / parent * predict
    expectNeg = parent_neg / parent * predict

    deviation = (value.pos - expectPos) ** 2 / expectPos \
                + (value.neg - expectNeg) ** 2 / expectNeg
    ...
    def decisiontree_iterator(parent):
        ...
        if further_fork == False:
            ...
            if(parent.pos > 0 and parent.neg > 0):
                ...
                df = len(parent.branches) - 1
                p_value = 1 - stats.chi2.cdf(DELTA, df)
...
def train_restaurant_tree(trainSet, testSet, N=200):
    ...
    for i in range(N, 0, -1):
        data = SyntheticRestaurant(i)
        sample_tree = DecisionTreeLearner(data)
        sample_error = err_ratio(sample_tree, testSet)
